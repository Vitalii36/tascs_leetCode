...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        ...
        def recursion_set(sr, sc, l, n, element):
            if sr >= l or sc >= n or sc < 0 or sr < 0:
                return
            if image[sr][sc] == element:
                image[sr][sc] = newColor
                recursion_set(sr + 1, sc, l, n, element)
                recursion_set(sr - 1, sc, l, n, element)
                recursion_set(sr, sc - 1, l, n, element)
                recursion_set(sr, sc + 1, l, n, element)

        recursion_set(sr, sc, l, n, element)
        return image
