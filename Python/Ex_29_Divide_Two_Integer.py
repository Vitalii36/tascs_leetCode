...
range: [−231,  231 − 1]. For this problem, assume that
...
class Solution:
    def divide(self, dividend, divisor):
        ...
        a = math.log(abs(dividend), LOG_BASE) - math.log(abs(divisor), LOG_BASE)
        output = max(min(int(math.pow(LOG_BASE, a)) * neg, INT_MAX), INT_MIN)
        return output
