...
class Solution:
    def shortestCompletingWord(self, licensePlate: str, words: List[str]) -> str:
        ...
        s = list(filter(lambda x: x in alphabet, licensePlate.lower()))
        sum_letter, len_word, res = 0, 0, False
        for word in words:
            sum_l = sum([s.count(x) if (x in word) and (word.count(x) >= s.count(x)) else 0 for x in set(s)])
            if sum_letter < sum_l:
                res = word
                sum_letter = sum_l
                len_word = len(word)
            elif sum_letter == sum_l and len_word > len(word):
                res = word
                sum_letter = sum_l
                len_word = len(word)
        return res
